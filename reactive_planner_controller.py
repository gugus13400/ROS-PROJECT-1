...
class ReactivePlannerController(PlannerControllerBase):
    ...
    def checkIfPathCurrentPathIsStillGood(self):
        ...
        # traversed If the route is not viable any more, call
        # self.controller.stopDrivingToCurrentGoal()
        path = self.currentPlannedPath

        for waypointNumber in range(-1, len(path.waypoints) - 1):
      		cell = path.waypoints[waypointNumber + 1]
        	xCoord = cell.coords[0]
		yCoord = cell.coords[1]
		if self.occupancyGrid.getCell(xCoord, yCoord) != 0:
                	return self.controller.stopDrivingToCurrentGoal()
